...
public final class GlyphNameplateRenderer implements NameplateRenderer {
    ...
    public void render(@Nonnull PlayerRef playerRef,
        ...
        NameplateManager.get().apply(playerRef.getUuid(), store, ref, " ");
        GlyphNameplateManager.get().apply(
                playerRef.getUuid(),
                world,
                store,
                ref,
                resolvedNameplate.getColoredText()
        );
    }
    ...
    public void clear(@Nonnull PlayerRef playerRef, @Nonnull World world) {
        Store<EntityStore> store = world.getEntityStore().getStore();
        GlyphNameplateManager.get().remove(playerRef.getUuid(), world, store);
